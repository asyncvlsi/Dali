...
void box_bin::update_cell_area(std::vector<block_al_t> &Nodelist) {
  /*
...
void box_bin::update_cell_area_white_space_LUT(std::vector< std::vector< int > > &grid_bin_white_space_LUT, std::vector<std::vector<grid_bin> > &grid_bin_matrix) {
  ...
  filling_rate = (double)total_cell_area/total_white_space;
}
...
void box_bin::update_terminal_list_white_space(std::vector<block_al_t> &Nodelist, std::vector< size_t > &box_terminal_list) {
...
void box_bin::update_ob_boundaries(std::vector<block_al_t> &Nodelist) {
...
bool box_bin::write_cell_in_box(std::string const &NameOfFile, std::vector<block_al_t> &Nodelist) {
  std::ofstream ost;
  ost.open(NameOfFile.c_str(), std::ios::app);
  if (ost.is_open()==0) {
    std::cout << "Cannot open file" << NameOfFile << "\n";
  ...
  for (auto &&cell_id: cell_list) {
    if (Nodelist[cell_id].is_movable()) {
...
bool box_bin::update_cut_point_cell_list_low_high(std::vector<block_al_t> &Nodelist, int &box1_total_white_space, int &box2_total_white_space) {
  ...
  double cut_line = 0;
  double ratio = 1 + box2_total_white_space/(double)box1_total_white_space;
  block_al_t *node;
  if (cut_direction_x) {
...
bool box_bin::update_cut_point_cell_list_low_high_leaf(std::vector<block_al_t> &Nodelist, int &cut_line_w, int std_cell_height) {
  int cell_area_low = 0;
  double cut_line = 0;
  ...
  if (cut_direction_x) {
    ...
    for (size_t i=0; i<cell_list.size(); i++) {
      ...
      cell_area_low_percentage = tmp_tot_cell_area_low/(double)total_cell_area;
    ...
    cut_line_w = left + (int)((total_cell_area_low/(double)total_cell_area)*(right - left));
  }
  return true;
}