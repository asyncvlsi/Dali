...
void circuit_t::initialize_HPWL_flags() {
  HPWLX_new = 0;
  HPWLY_new = 0;
  HPWLX_old = 1e30;
  HPWLY_old = 1e30;
...
void circuit_t::build_problem_clique_x() {
...
void circuit_t::build_problem_clique_y() {
  // before build a new Matrix, clean the information in existing matrix
  for (size_t i=0; i<CELL_NUM; i++) {
    Ay[i][0].weight = 0;
    // make the diagonal elements 0
...
void circuit_t::update_max_min_node_x() {
  // update the y direction max and min node in each net
  float max_pin_loc_x, min_pin_loc_x;
  size_t maxpindex_x, minpindex_x;
  HPWLX_new = 0;
...
void circuit_t::build_problem_b2b_x() {
  // before build a new Matrix, clean the information in existing matrix
  for (size_t i=0; i<CELL_NUM; i++) {
    Ax[i][0].weight = 0;
    // make the diagonal elements 0
...
void circuit_t::build_problem_b2b_x_nooffset() {
  // before build a new Matrix, clean the information in existing matrix
  for (size_t i=0; i<CELL_NUM; i++) {
    Ax[i][0].weight = 0;
    // make the diagonal elements 0
...
void circuit_t::update_max_min_node_y() {
  // update the y direction max and min node in each net
  float maxpinlocy, minpinlocy;
  size_t maxpindex_y, minpindex_y;
  HPWLY_new = 0;
...
void circuit_t::build_problem_b2b_y() {
  // before build a new Matrix, clean the information in existing matrix
  for (size_t i=0; i<CELL_NUM; i++) {
    Ay[i][0].weight = 0;
    // make the diagonal elements 0
...
void circuit_t::build_problem_b2b_y_nooffset() {
  // before build a new Matrix, clean the information in existing matrix
  for (size_t i=0; i<CELL_NUM; i++) {
    Ay[i][0].weight = 0;
    // make the diagonal elements 0
...
void circuit_t::CG_solver(std::string const &dimension, std::vector< std::vector<weight_tuple> > &A, std::vector<float> &b, std::vector<size_t> &k) {
...
void circuit_t::CG_solver_x() {
  CG_solver("x", Ax, bx, kx);
}

void circuit_t::CG_solver_y() {
  CG_solver("y", Ay, by, ky);
}

void circuit_t::cg_close() {
  Ax.clear();
  Ay.clear();
  bx.clear();
  by.clear();
