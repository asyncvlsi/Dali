...
template<class T>
DisplaceViewer<T>::DisplaceViewer() = default;

template<class T>
inline void DisplaceViewer<T>::SetSize(int sz) {
  sz_ = sz;
  x_.resize(sz);
  y_.resize(sz);
  u_.resize(sz);
...
template<class T>
inline void DisplaceViewer<T>::SetXY(int i, T x_val, T y_val) {
  x_[i] = x_val;
  y_[i] = y_val;
}
...
template<class T>
void DisplaceViewer<T>::SaveDisplacementVector(std::string const &name_of_file) {
