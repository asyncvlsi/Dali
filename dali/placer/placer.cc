...
void Placer::SetNumThreads(int num_threads) {
  if (num_threads < 1) {
    num_threads = 1;
    DaliWarning("Number of threads ("
                << num_threads
                << ") for placer is less than 1, using 1 instead");
  }
...
double Placer::PlacementDensity() const { return placement_density_; }
...
void Placer::SetBoundary(int left, int right, int bottom, int top) {
  ...
  DaliExpects(left < right,
              "Invalid boundary setting: left boundary should be less than "
              "right boundary!");
