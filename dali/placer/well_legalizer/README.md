...
`-well_tap_pattern` option selects how those taps are arranged in each row, and
a pattern-agnostic coverage check verifies the rule geometrically whatever the
pattern.

Each diagram carries its own legend. The P+/N+ layer is called the *implant*
layer throughout; some PDKs call it "select". A well-tap cell is short and
straddles the row's P/N boundary so it ties both wells at once, and the rest of
its column is filled with implant, which is required rather than cosmetic.

Cell and row heights deliberately differ: gridded cells are quantized in height,
so a row is exactly tall enough for its tallest cell and each row's P/N boundary
sits at a different offset. Adjacent rows are flipped so like wells abut.
...
What a tap pattern must satisfy for the output to be DRC clean. New patterns
have to keep all of these true.
...
Requirement 2 is the one that is easy to get wrong: placing the tap correctly is
not enough, the implant layer around it must be left gap-free.
