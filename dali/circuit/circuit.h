...
class Circuit {
 ...
 public:
  ...
  int32_t Micron2DatabaseUnit(double x) const;

  // convert length from database unit to um
  double DatabaseUnit2Micron(int32_t x) const;

  // Dali to PhyDB distance scale factor x
  int32_t DistanceScaleFactorX() const;

  // Dali to PhyDB distance scale factor y
  int32_t DistanceScaleFactorY() const;

  // convert location in a design from Dali to PhyDB
  int32_t LocDali2PhydbX(double loc) const;

  // convert location in a design from Dali to PhyDB
  int32_t LocDali2PhydbY(double loc) const;

  // convert location in a design from PhyDB to Dali
  double LocPhydb2DaliX(int32_t loc) const;

  // convert location in a design from PhyDB to Dali
  double LocPhydb2DaliY(int32_t loc) const;
  ...
  void SetDatabaseMicrons(int32_t database_micron);

  // get database microns
  int32_t DatabaseMicrons() const;
  ...
  int32_t RowHeightGridUnit() const;
  ...
  std::unordered_map<std::string, int32_t> &MetalNameMap();
  ...
  int32_t GetMetalLayerId(std::string const &metal_name);
  ...
  int32_t GetBlockId(std::string const &block_name);
  ...
  int32_t GetIoPinId(std::string const &iopin_name);
  ...
  int32_t GetNetId(std::string const &net_name);
  ...
  void ReportHPWLHistogramLinear(int32_t bin_num = 10);

  // report the histogram of HPWL using logarithmic bins
  void ReportHPWLHistogramLogarithm(int32_t bin_num = 10);
 ...
 private:
  ...
  void SetBoundary(int32_t left, int32_t bottom, int32_t right, int32_t top);
  ...
  void ExportIoPins(std::ofstream &ost, int32_t mode);
  void ExportAllNets(std::ofstream &ost);
  void ExportPowerNetsForWellTapCells(std::ofstream &ost);
  void ExportNets(std::ofstream &ost, int32_t mode);
};
