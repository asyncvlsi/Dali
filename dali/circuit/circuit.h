...
/** Tunable constants shared across Circuit computations. */
...
/**
 * Top-level circuit model joining technology data and design data.
 *
 * Technology comes from LEF/CELL sources and design data comes from DEF. The
 * usual entry point is InitializeFromPhyDB(), but the public add/set APIs also
 * support constructing a circuit programmatically in the same order as DEF/LEF
 * loading: units and grid, technology layers/macros, design capacities,
 * components, I/O pins, nets, and well geometry.
 */
class Circuit {
  friend class Placer;
 ...
 public:
  ...
  /** Convert length from LEF/DEF database units to microns. */
  ...
  /** Return LEF database microns. */
  int DatabaseMicrons() const;

  /** Set manufacturing grid in microns. */
