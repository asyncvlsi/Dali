...
class Net {
  protected:
    ...
    double
        inv_p_; // 1.0/(p-1), where p is the number of pins connected by this net
  ...
  public:
    ...
    int Pnum() const { return (int) blk_pin_list.size(); }
    int FixedCnt() const { return cnt_fixed_; }
    void SetAux(NetAux *aux) {
        DaliExpects(aux != nullptr,
                    "Cannot set @param aux to nullptr in void Net::SetAux()\n");
        aux_ptr_ = aux;
    }
