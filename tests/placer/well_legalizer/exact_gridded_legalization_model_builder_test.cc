...
TEST(ExactGriddedLegalizationModelBuilderTest,
     SupportsLegacyImpliedSingleWellRegion) {
  Circuit circuit;
  circuit.SetManufacturingGrid(1);
  circuit.SetUnitsDistanceMicrons(1);
  circuit.SetGridValue(1, 1);
  circuit.ReserveSpaceForDesignImp(1, 0, 0);
  Macro* macro = circuit.AddMacro("legacy_cell", 2, 3);
  macro->AddNwellRect(0, 1, 2, 3);
  circuit.AddComponent("component", "legacy_cell", 0, 0, PLACED);
  ...
  ASSERT_EQ(model.cells[0].regions.size(), 1U);
  EXPECT_EQ(model.cells[0].regions[0].p_well_height, 1);
  EXPECT_EQ(model.cells[0].regions[0].n_well_height, 2);
