...
TEST(OrToolsGriddedStripeOptimizerTest,
     PrioritizesTheBandWithLargerAffectedNetHpwl) {
  if (!OrToolsCompactGriddedLegalizer::IsAvailable()) {
    GTEST_SKIP() << "Dali was built without OR-Tools 9.15.x";
  }
  ...
  circuit.AddComponent("high", "cell", 4, 2, PLACED);
  circuit.AddComponent("near_anchor", "cell", 3, 0, FIXED);
  circuit.AddComponent("far_anchor", "cell", 30, 2, FIXED);
  ...
  EXPECT_EQ(result.stripes[0].column_index, 1);
  EXPECT_EQ(result.stripes[1].column_index, 0);
  EXPECT_GT(result.stripes[0].initial_priority_hpwl,
            result.stripes[1].initial_priority_hpwl);
}
