...
std::pair<double, double> TransformPinPoint(double px, double py,
  ...
  switch (orient) {
    ...
    case FE:
      return {-py, -px};
    default:
      DaliExpects(false, "Unknown Dali orientation");
      return {px, py};
  }
}
...
PinBbox GetPlacedPinBbox(phydb::IOPin& iopin, double extra_margin = 0.0) {
  ...
  const double urx = std::max(shape.LLX(), shape.URX()) + extra_margin;
  const double ury = std::max(shape.LLY(), shape.URY()) + extra_margin;

  std::array<std::pair<double, double>, 4> corners{
      std::make_pair(llx, lly), std::make_pair(llx, ury),
      std::make_pair(urx, lly), std::make_pair(urx, ury)};

  PinBbox bbox{static_cast<double>(x), static_cast<double>(y),
               static_cast<double>(x), static_cast<double>(y)};
  bool is_first_point = true;
  for (const auto& corner : corners) {
    const auto transformed =
        TransformPinPoint(corner.first, corner.second, orient);
    const double abs_x = x + transformed.first;
    const double abs_y = y + transformed.second;
    if (is_first_point) {
      bbox = {abs_x, abs_y, abs_x, abs_y};
      is_first_point = false;
    } else {
      bbox.lx = std::min(bbox.lx, abs_x);
      bbox.ly = std::min(bbox.ly, abs_y);
      bbox.ux = std::max(bbox.ux, abs_x);
      bbox.uy = std::max(bbox.uy, abs_y);
    }
  }
  return bbox;
}
 ...
 *
 * The benchmark used for these testcases come from ISPD2019, and in this
 * benchmark, all I/O pins are PLACED/FIXED. In order to test the I/O placer, we
 * will have to modify this benchmark and make all iopins UNPLACED. This
 * function changes the placement status of all iopins to UNPLACED.
 *
 * @param p_phydb : the PhyDB instance containing the design with iopins to be
 * placed
 */
...
bool IsEveryIoPinPlacedOnBoundary(phydb::PhyDB* p_phydb) {
  ...
  for (auto& iopin : iopins) {
    bool is_on_boundary =
        IoPinPlacedOnBoundary(iopin, left, right, bottom, top);
    if (!is_on_boundary) {
      res = false;
    }
  }
...
bool IsIoPinNoSpacingViolation(phydb::PhyDB* p_phydb, phydb::IOPin& io_pin0,
                               phydb::IOPin& io_pin1) {
  if (io_pin0.GetLayerName() != io_pin1.GetLayerName()) return true;
...
bool IsNoIoPinOverlapAndSpacingViolation(phydb::PhyDB* p_phydb) {
  ...
  for (size_t i = 0; i < sz; ++i) {
    ...
    for (size_t j = i + 1; j < sz; ++j) {
      ...
      bool is_spacing_satisfied =
          IsIoPinNoSpacingViolation(p_phydb, io_pin0, io_pin1);
...
bool IsEveryIoPinManufacturable(phydb::PhyDB* p_phydb) {
  ...
  for (auto& iopin : iopins) {
    ...
    DaliExpects(
        AbsResidual(tmp_grid, 1.0) < 1e-5,
        "DATABASE_MICRONS * MANUFACTURINGGRID is not close to an integer?");
    int grid = static_cast<int>(tmp_grid);
    bool is_manufacturable =
        (shape.LLX() % grid == 0) && (shape.LLY() % grid == 0) &&
        (shape.URX() % grid == 0) && (shape.URY() % grid == 0);
...
bool IsEveryIoPinInsideDieArea(phydb::PhyDB* p_phydb) {
  ...
  for (auto& iopin : iopins) {
    PinBbox bbox = GetPlacedPinBbox(iopin);

    bool is_in_die_area =
        (bbox.lx >= die_area.LLX() && bbox.lx <= die_area.URX()) &&
        (bbox.ux >= die_area.LLX() && bbox.ux <= die_area.URX()) &&
        (bbox.ly >= die_area.LLY() && bbox.ly <= die_area.URY()) &&
